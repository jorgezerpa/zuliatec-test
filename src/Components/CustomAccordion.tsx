...
export const CustomAccordion = ({children, title, icon }:{children?:ReactNode, title:string, icon?:any}) => {
  ...
  return (
    <div style={{ fontSize:'14px' }}>
      <div onClick={handleToggleCollapse} className='d-flex align-items-center'>
        <div className='pr-1 d-flex align-items-center'>{ icon && icon }</div>
        <div>{ title }</div>
      </div>
        <CCollapse visible={showCollapse} >
          <div className='px-3'>
            { children }
          </div>
        </CCollapse>
